/**
 * Cookie Consent Banner - Arquiteto do Tempo (Embedded Version)
 * RGPD Compliant Cookie Manager
 * Version: 1.0.0-embedded
 * Date: 09 Janeiro 2026
 *
 * IMPORTANTE: Esta versão é para usar DENTRO do Systeme.io
 * - NÃO carrega GTM (assume que já existe no domínio pai)
 * - Apenas envia eventos para dataLayer existente
 * - Use cookie-banner.js para Netlify standalone
 */
...
(function() {
    ...
    const CookieConsent = {
        ...
        loadGoogleAnalytics: function() {
            // EMBEDDED VERSION: Apenas envia evento para dataLayer existente
            // GTM já deve estar carregado no Systeme.io

            // Verifica se dataLayer existe (GTM no domínio pai)
            if (!window.dataLayer) {
                console.warn('⚠️ Cookie Banner: dataLayer não encontrado. GTM deve estar no Systeme.io.');
                window.dataLayer = [];
            }

            // Push consent event para GTM existente
            window.dataLayer.push({
                'event': 'cookie_consent_analytics',
                'analytics_consent': true
            });

            console.log('✅ Cookie Consent: Analytics aceite, evento enviado para GTM');
        },

        loadMarketingScripts: function() {
            // EMBEDDED VERSION: Apenas envia evento para dataLayer
            // Marketing scripts devem estar configurados como tags no GTM

            if (!window.dataLayer) {
                window.dataLayer = [];
            }

            // Push consent event para Marketing
            window.dataLayer.push({
                'event': 'cookie_consent_marketing',
                'marketing_consent': true
            });

            console.log('✅ Cookie Consent: Marketing aceite, evento enviado para GTM');
        },
